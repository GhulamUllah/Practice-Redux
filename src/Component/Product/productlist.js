...
import {
  deleteproduct,
  getproduct,
} from "../../Redux/reducer/action/productactions";
export default function Productlist({ setauth }) {
  ...
  let handledelete = async (p, id) => {
    ...
    setloading(true);
    try {
      let det = await axios.delete(
        `http://localhost:5000/product/delete-product/${p._id}`
      );
            ...
            return (
              <div className="card">
                <img
                  ...
                  alt="Error Internet Connection"
                  width="200px"
                  height="250px"
                />
                <Typography>Product Tilte: {e.productTitle}</Typography>
                <Typography>
                  Product Description: {e.productDescription}
                </Typography>
